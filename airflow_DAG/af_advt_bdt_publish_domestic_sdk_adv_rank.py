...
wf_data_path = "/apps/hive/warehouse/bdt_adv.db/bdt_publish_domestic_sdk_adv_rank"
...
delete_hdfs_file_2 = BashOperator(
    ...
    bash_command='hadoop fs -rm -r -f ' + wf_data_path + '/dt={{ next_ds }}/_SUCCESS',
    dag=dag
)
...
af_adt_advertiser_revenue_report_ag_hql = HiveOperator(
    ...
    hql=open(af_hql_path + "/hql_bdt_publish_domestic_sdk_adv_rank.hql", 'r').read().replace("hivevar", "hiveconf"),
...
create_hdfs_file_2 = BashOperator(
    ...
    bash_command='hadoop fs -touchz ' + wf_data_path + '/dt={{ next_ds }}/_SUCCESS',
    dag=dag
)
